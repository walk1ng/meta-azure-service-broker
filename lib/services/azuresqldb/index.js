...
Handlers.update = function (params, next) {
  log.debug('niroy4 sqldb/index/update/params: %j', params);
  var sqldbOps = new sqldbOperations(params.azure);
  var cp = new cmdUpdate(params);
  cp.update(sqldbOps, function (err, result) {
    if (err) {
      return common.handleServiceError(err, next);
    }

    var replyCode = result.statusCode;
    // Azure 201 Created should be treated as 200 OK by  CF cloud controller
    if (replyCode === HttpStatus.CREATED){
      replyCode = HttpStatus.OK;
    }
    var reply = {
      statusCode: replyCode,
      code: HttpStatus.getStatusText(replyCode),
      value: {'some':'json'} // Can't be empty
    };


    log.debug('sqldb/index/update/reply: %j', reply);
    next(null, 'update', reply, result);
  });
};
