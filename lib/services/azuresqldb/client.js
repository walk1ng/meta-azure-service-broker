...
sqldbOperations.prototype.createOrUpdateServer = function (parameters, callback) {

    var that = this;
    log.debug('sqldb client: Entering createOrUpdateServer with parameters: %j', parameters);
    ...
    var headers = common.mergeCommonHeaders('sqldb client - createOrUpdateServer', that.standardHeaders);
    msRestRequest.PUT(that.serverUrl, headers, sqlServerParameters, API_VERSIONS.SQL, function (err, res, body) {
        if (err) {
            log.info('sqldb client: createOrUpdateServer: err %j', err);
            return callback(err, null);
        }
        ...
        if (result.statusCode != HttpStatus.CREATED && result.statusCode != HttpStatus.OK) {
            return common.formatErrorFromRes(res, callback);
        } else {
            log.debug('sqldb client: createOrUpdateServer: body: %j', body);
            result.body = body;
            callback(null, result);
        }
    });
