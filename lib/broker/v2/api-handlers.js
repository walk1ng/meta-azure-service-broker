...
Handlers.handleUpdateRequest = function(broker, req, res, next) {
  var instanceId = req.params.instance_id;
  broker.log.info('handleUpdateRequest - for (instanceId: %s) from %s:%s', instanceId, req.connection.remoteAddress, req.connection.remotePort);
  broker.log.info('niroy2 handleUpdateRequest - params: %j\nreq: %s\n res:%s', req.params, req, res);
  
  var processResponse = function(error, lastOperation, reply, result) {
    broker.log.debug('handleUpdateRequest - lastOperation=%s reply=%j, result=%j',  lastOperation, reply, result);
    if (error) {
      broker.log.error('handleUpdateRequest - Failed in sending update request for (instanceId: %s) to Azure. Error: %j', instanceId, error);
      res.send(error.statusCode, error);

      broker.log.info('handleUpdateRequest - Sent the update error for (instanceId: %s) to cc.', instanceId);
      return next();
    }
    res.send(reply.statusCode, reply.value);
    broker.log.info('handleUpdateRequest - Succeeded in sending the update response (%j) for (instanceId: %s) to cc.', reply.value, instanceId);
    return next();
  };
  ...
  params.accountPool = accountPool;
  emitEvent(broker, operation, params, processResponse, next);
  //TODO update DB params on success
};
