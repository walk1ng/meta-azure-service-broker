...
Handlers.handleUpdateRequest = function(broker, req, res, next) {
  ...
  log.info('niroy2 handleUpdateRequest - params: %j\nreq: %s\n res:%s', req.params, req, res);
  ...
  async.waterfall([
    ...
    function (reply, newParams, callback) {
    ...
    function (err, reply) {
      if (err) {
        log.error('handleUpdateRequest - Failed in sending update request for (instanceId: %s) to Azure. Error: %j', instanceId, err);        
        res.send(err.statusCode, err);
        return next();
      }
